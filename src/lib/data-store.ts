    ...
    menuItems: [
      {
        ...
        categories: ["热菜", "荤菜"],
        ingredientTags: ["五花肉", "青椒"],
        imageUrl: null,
        recipe: {
          id: createId("recipe"),
          menuItemId: menuItemId1,
          title: "家常快炒",
          ingredientNotes: "先煮后炒，肥而不腻。",
          steps: "先将五花肉煮到八成熟，切片后与青椒一同翻炒。",
          createdAt: now,
          updatedAt: now,
        },
        createdAt: now,
      ...
      },
      {
        ...
        categories: ["热菜", "荤菜"],
        ingredientTags: ["牛腩", "白萝卜"],
        imageUrl: null,
        recipe: {
          id: createId("recipe"),
          menuItemId: menuItemId3,
          title: "慢炖招牌",
          ingredientNotes: "牛腩与萝卜一起炖到软烂入味。",
          steps: "焯水后小火慢炖 90 分钟以上。",
          createdAt: now,
          updatedAt: now,
        },
        createdAt: now,
    ...
    sessions: [
      {
        ...
        name: "本周试运行菜单",
        slug: "demo-session",
        description: "本地演示会话。没有 DATABASE_URL 时会自动使用本地 JSON 数据。",
        isActive: true,
        startsAt: now,
        endsAt: null,
        createdAt: now,
        ...
        items: {
          create: input.items.map((item) => {
            ...
            return {
              ...
              itemName: menuItem?.chineseName || menuItem?.name || "菜单项",
              quantity: item.quantity,
              note: item.note,
...
export async function updateOrderItemStatus(input: {
  ...
  if (hasDatabase) {
    const updatedItem = await db.orderItem.update({
      ...
      data: {
        status: input.status,
        rejectionReason: input.status === "REJECTED" ? input.rejectionReason || "暂不可用" : null,
      },
      select: { orderId: true },
    });
